...
| Sweet Potato KeyStation with a provisional Waveshare 7.9-inch HDMI/USB touch display | Admit selected fades and eased transitions | Native Meson display/input and restricted Mali-450 renderer proposed; exact panel mode and frame/memory budgets require acceptance |
...
The [Sweet Potato/KeyStation design](09_sweet_potato_keystation.md) develops the Amlogic S905X path: CPU reference rendering, native Meson HDMI and USB touch, then restricted Mali-450 acceleration. The provisional Waveshare 7.9-inch panel needs portrait scanout and matching render/input rotation for horizontal mounting. Exact SKU and mode compatibility remain open. Fidelity.Platform now has a [board entry and source inventory](../../Fidelity.Platform/Hardware/Products/LibreComputer/AML_S905X_CC_V2/README.md), including the Allwinner H6 mismatch in Composer's existing Blinky sample. Video decoding remains separate from UI graphics, and hosted Linux support supplies reference evidence for a further native driver port.
