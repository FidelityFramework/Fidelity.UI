...
Research review, 18 September 2026. This is a proposal for discussion, not an adopted API or an implementation claim. Fable.Ripple and the other F# libraries are research references. No reference-library adoption or changes to the compiler/native UI runtime are part of this review. Code examples below are proposed Clef-shaped notation unless explicitly identified as existing source. A separate [draft site experiment](/home/hhh/repos/clef-lang-site/hugo/content/blog/pitch-slew-and-demand.md) implements browser pitch smoothing to illustrate the demand and control ideas; it is not a Fidelity runtime or measured firmware implementation.
...
**The primary native direction is a new Clef-native reactive-area engine.** This incorporates the user's clarification during the review: both existing native and WREN UI work are experimental, and neither has an architectural veto. LVGL is chiefly a component-model reference, with additional lessons about embedded scope and constraints; it is not a selected engine or an assumed internal design. Solid's component composition is another direct reference. The native-area model needs to be designed and measured in its own right.
...
| Motion | Shared declarative fades/easing/transitions, activated under an owner and product capability policy | Target measurements, layer budgets and user motion preferences |
...
| [HelloDISCO acceptance](/home/hhh/repos/MCU/ST/STM32H747I-DISCO/HelloDISCO/evidence/hardware/2026-09-13-interactive-initial-palette/README.md) | Recorded M7 banner, joystick/LED and palette operation, including debugger-disconnected cold start and frame readback | Audio, autonomous M4 operation, touch or a general reactive-area engine |
...
| Solid components | Nested functions, reactive properties/children, instance setup separate from binding updates | DOM/JSX realization is not the portable component contract or cold-construction policy |
| LVGL | Retained widget composition, parts/states/styles, layouts, events and transitions | Its mutable C object API and rendering internals need not define Clef's cold functional facade |
...
The component goal is practical: native views should be as straightforward to compose as web views. Named functions with typed properties, actions and child content should build reusable panels and controls using rows, columns, grids and overlays. Styling, state variants and motion should compose through that same surface. LVGL provides a concrete widget tree with common styles/events; Solid shows reusable function components whose setup and selective bindings have different execution lifetimes. Fidelity can combine these lessons with cold construction, pure projections and owned internal mutation. See [LVGL widget tree](https://lvgl.io/docs/open/9.4/details/common-widget-features/tree.html), [styles](https://lvgl.io/docs/open/9.4/details/common-widget-features/styles/overview.html), [events](https://lvgl.io/docs/open/9.4/details/common-widget-features/events.html), [Solid components](https://docs.solidjs.com/concepts/components/basics) and [properties/children](https://docs.solidjs.com/concepts/components/props). This authoring goal is compatible with a native engine; it does not require adopting HTML/CSS or LVGL object handles as the public API.
...
**Motion belongs in the common component model, with target-specific admission and cost.** Fades, easing and state transitions should be ordinary declarative options alongside layout and style. A cold transition starts no timer until an owner activates it. Its clock demand then drives the affected presentation work until completion, cancellation or explicit suspension. Define elapsed-time sampling, duration/easing, property precedence, retargeting from the current presented value and distinct completion/cancellation outcomes. The authoritative application target remains separate from interpolated presentation values. LVGL's [animation model](https://lvgl.io/docs/open/9.4/details/main-modules/animation.html) and [state transitions](https://lvgl.io/docs/open/9.4/details/common-widget-features/styles/transitions.html) offer concrete timing and lifecycle references, without dictating the native scheduler.

The cost follows the affected stages. Animating size may cause layout each frame; color may require painting; a fade or transform can reuse retained content only when group opacity, clipping and blending are represented correctly. Retained layers consume storage and can become stale when content changes. Bound textures/layers, uploads, pending frames and fallback work instead of creating a GPU layer per component. A browser may realize the same intent through its animation facilities; equivalent lifecycle and interruption behavior matter more than identical frame counts. Reduced or disabled decorative motion preserves the final state and essential feedback. Input/focus and removal policies remain explicit during fades, and cancellation does not release storage still used by a renderer.

| Product profile | Intended motion policy | Current boundary |
|---|---|---|
| STM32H7 / HelloDISCO instrument | Omit decorative UI motion; apply state changes directly | A product choice that protects the selected workload budget, not a claim that MCU animation is impossible |
| Sweet Potato KeyStation with the selected Waveshare 7.9inch HDMI LCD, SKU 17916 | Admit selected fades and eased transitions | Native Meson display/input and restricted Mali-450 renderer proposed; exact panel mode and frame/memory budgets require acceptance |
| GPU desktop and capable WREN hosts | Richer transitions and retained composition | Capabilities, resource completion and user motion preferences remain explicit; present demos do not establish this engine |

The STM32H7 choice concerns decorative UI animation. Live measurements and DSP pitch/control smoothing remain separate behavioral requirements.

The [Sweet Potato/KeyStation design](09_sweet_potato_keystation.md) develops the Amlogic S905X path: CPU reference rendering, native Meson HDMI and USB touch, then restricted Mali-450 acceleration. The selected Waveshare 7.9inch HDMI LCD, SKU 17916 (ASIN B087CNJYB4), uses portrait scanout with matching render/input rotation for horizontal mounting. S905X timing and touch-driver acceptance remain implementation work. Fidelity.Platform's [board entry and source inventory](../../Fidelity.Platform/Hardware/Products/LibreComputer/AML_S905X_CC_V2/README.md) pin AML-S905X-CC-V2, its DDR4 memory generation, Linux device-tree identity and U-Boot configuration. Video decoding remains separate from UI graphics, and hosted Linux support supplies reference evidence for a further native driver port.

**HelloDISCO is a concrete future test host for continuous instrument demand beside reactive controls.** Its accepted image has an immutable indexed banner and a foreground palette consumer on M7. The proposed Minimoog-like synthesizer and the user's range-sensing theremin mode add substantially different workloads; they are future experiments. The theremin discussion considers optical or ultrasonic range sensing, not a commitment to RF heterodyning. It accepts pitch smoothing as a design direction while leaving volume smoothing a separate choice, potentially at the generator. Treat that conversation as requirements exploration, not verified sensor, timing or register guidance. See the [accepted checkpoint](/home/hhh/repos/MCU/ST/STM32H747I-DISCO/HelloDISCO/evidence/hardware/2026-09-13-interactive-initial-palette/README.md), [instrument design](/home/hhh/repos/Fidelity.Platform/docs/STM32H7_SYNTH_DESIGN.md), and [user-supplied exploratory conversation](https://share.gemini.google/gCokm9b9ODz1).

Once explicitly activated, an audio service must keep producing sample blocks even when parameters are unchanged or its panel is closed. That active service demand is independent of visual observation: knobs, calibration panels and waveform areas can update at their own bounded rates, with unaffected layout and paint skipped. Opening a panel observes a coherent instrument snapshot; retiring it removes its subscriptions without implicitly stopping audio. No audio deadline may depend on a paint completing. A future M7 audio/M4 control split needs its own boot, publication and peripheral-ownership acceptance; it is not a property of the UI syntax or the accepted image.

Use typed calibration and control values to distinguish sensor distance, elapsed measurement time, pitch/frequency, gain and smoothing time, with explicit ranges, units and invalid/stale-sample policy. Replaceable control presentation should edit that behavior contract. Continuous parameter snapshots can use latest-value delivery and coherent block-boundary application; note-on/off and other event occurrences need a separate bounded ordering and overflow policy. Sample history needed by a plot is also distinct from the latest parameter value. These are useful tests of headless behavior, service ownership and reactive-area lifetime across both native and DOM hosts.

The instrument scaffold proposes 48 kHz audio in 64-frame blocks: about 1.333 ms per refill, with a proposed 1 ms work budget after reserve. These are design targets, not measured acceptance or guaranteed deadlines. Validation must include audio-buffer ownership, bounded control work and display interference on the selected hardware. A cold graph can avoid unused work, but first activation and any admitted preparation must fit their own latency and memory budgets. See [proposed scheduling budgets](/home/hhh/repos/Fidelity.Platform/docs/STM32H7_SYNTH_DESIGN.md#scheduling-and-initial-budgets).
...
Motion cases should interrupt and retarget a fade, resize during movement, remove an animating control, and change reduced-motion policy mid-transition. Verify final state, hit/focus behavior, callback outcomes, clock-demand retirement and external buffer release. Compare immediate presentation on the STM32H7 product profile with admitted motion on a richer host using the same component description; measure frame pacing and retained-layer cost rather than inferring acceleration from an HDMI connector.
...
| HelloDISCO | `94dddd4` | Existing settings/model/test work preserved; acceptance records and future instrument scaffold inspected; repository not edited |
...
| Fidelity.Platform | `0410249` | Source inspected; display and instrument design wording aligned; hardware code and acceptance unchanged |
